...
<html lang="en">
  <head>
  	<title>Espace Ville/Etudiant</title>
    <meta charset="utf-8">
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var data = google.visualization.arrayToDataTable([
         ...
         <?php
         $sql = "SELECT distinct ville, count(id)  FROM etudiant GROUP BY ville ";
         $fire = mysqli_query($con,$sql);
          while ($result = mysqli_fetch_assoc($fire)) {
            echo"['".$result['ville']."',".$result['count(id)']."],";
          }
...
.ville{
    ...
    <?php
        ...
        ?>
        <form method="GET" action="controller/loadEtudiant.php" >
        <fieldset>

        Selectionner une ville : <select name="ville" class="ville">
            <?php
            foreach ($es->findVilles() as $v) {
                ?>
       ...
       <br> <hr><br>
         ...
         <table border="1" class="tab">
            ...
            <tbody>
                ...
                $es = new EtudiantService();
                if(isset($_GET['ville']))
                foreach ($es->findEtudiantsByVille($_GET['ville']) as $e) {
                    ?>
                    <tr>
                        <td><?php echo $e->getId(); ?></td>
                        <td><?php echo $e->getNom(); ?></td>
                        <td><?php echo $e->getPrenom(); ?></td>
                        <td style="color: #153f00; font-weight: bold;"><?php echo $e->getVille(); ?></td>*
